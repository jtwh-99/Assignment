...
{		
 	...
 	{	 
		...
		bs.ClearError();
 		...
 		//Get values
         var Size = $('.Screen_Data').find('.Size');
         var Quantity = $('.Screen_Data').find('.Quantity');
         var Occasion = $('.Screen_Data').find('.Occasion');
         
		var TaskDate = $('.Screen_Data').find('.TaskDate');
 		var TaskDesc = $('.Screen_Data').find('.TaskDesc');

 		if(frm.IsEmpty(Size.val() ))
		{
			//Show alert
			bs.ShowError ("Please select boutiquet size",Size)
		}else if(frm.IsEmpty(Quantity.val()))
		{
			//Show alert
			bs.ShowError ("Please select quantity of floral",Quantity)
		}else if(frm.IsEmpty(Occasion.val()))
		{
			//Show alert
			bs.ShowError ("Please specifiy occasion",Occasion)
		}
		else if(frm.IsEmpty(TaskDate.val()))
		{
			//Show alert
			bs.ShowError ("Please specific date",TaskDate)
		}
	...
	$(document).on('click', '.BTN_View', function(event) 
 	{
 		...
 		strTableData += '<thead>';
 		//strTableData += '<tr>';
 		strTableData += '<th>No</th>';
         strTableData += '<th>Boutiquet Size</th>';
         strTableData += '<th>Quantity</th>';
         strTableData += '<th>By Occasion</th>';
         
 		strTableData += '<th>Request Date</th>';
 		strTableData += '<th>Add-Ons Items</th>';
 		...
 		for (var i = 0; i < AllTasks.length; i++) 
 		{
            ...
            strTableData += '<td>'+val.Size+'</td>';
            strTableData += '<td>'+val.Quantity+'</td>';
            strTableData += '<td>'+val.Occasion+'</td>';
	...
	$(document).on('click', '.BTN_Edit_Entry', function(event) 
 	{
		...
		{		
         ...
         var Size = $('.Screen_Data').find('.Size').val(data[0].Size);
         var Quantity = $('.Screen_Data').find('.Quantity').val(data[0].Quantity);
         var Occasion = $('.Screen_Data').find('.Occasion').val(data[0].Occasion);
	...
	$('.Screen_Data').on('click', '.BTN_Update_Task', function(event) 
 	{	 
		...
		{
			//Show alert
			bs.ShowError ("Please select the boutiquet size",Size)
		}else if(frm.IsEmpty(Quantity.val()))
		{
			//Show alert
			bs.ShowError ("Please select quantity of floral",Quantity)
		...
		{
			//Show alert
			bs.ShowError ("Please specific occasion",Occasion)
		}
		else if(frm.IsEmpty(TaskDate.val()))
		{
			//Show alert
		...
		{		
			...
			{		
				...
				{ 
                    val.Size = Size.val();
                    val.Quantity = Quantity.val();
                    val.Occasion = Occasion.val();
